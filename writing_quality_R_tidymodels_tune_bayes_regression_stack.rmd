...
prediciton_model_rec <- recipe(score ~ ., data = train_data %>% select(-score_factor))

prediciton_model_rec_glm <- recipe(score ~ ., data = train_data %>% select(-score_factor)) %>%
  step_naomit(all_numeric_predictors()) %>%
  step_pca(all_numeric_predictors())
```
...
xgb_best_workflow <- xgb_wf %>% update_model(xgb_best_model) %>% update_recipe(prediciton_model_rec)
xgb_best_fit <- fit(xgb_best_workflow, data = train_data %>% select(-score_factor))
...
lgbm_best_workflow <- lgbm_wf %>% update_model(lgbm_best_model) %>% update_recipe(prediciton_model_rec)
lgbm_best_fit <- fit(lgbm_best_workflow, data = train_data %>% select(-score_factor))
...
rf_best_workflow <- rf_wf %>% update_model(rf_best_model) %>% update_recipe(prediciton_model_rec)
rf_best_fit <- fit(rf_best_workflow, data = train_data %>% select(-score_factor))
...
lr_best_workflow <- lr_wf %>% update_model(lr_best_model) %>% update_recipe(prediciton_model_rec_glm)
lr_best_fit <- fit(lr_best_workflow, data = train_data %>% select(-score_factor))
...
knn_best_workflow <- knn_wf %>% update_model(knn_best_model) %>% update_recipe(prediciton_model_rec)
knn_best_fit <- fit(knn_best_workflow, data = train_data %>% select(-score_factor))
...
models_rmse <- cbind(xgb_rmse, lgbm_rmse, rf_rmse, lr_rmse, knn_rmse)
...
Missing <- setdiff(lag_cols, names(test_logs_adj)) # Find names of missing columns (mostly due to test data)
test_logs_adj[Missing] <- 0 # Add them, filled with '0's (mostly due to test data)
...
```{r}
test_df_reduced_2 <- filtered_test_df %>%
  select(-c(
    time,
    event_ArrowDown,
    event_ArrowLeft,
...
```{r}
test_df_reduced_2 <- test_df_reduced_2 %>% clean_names()
```


```{r}
scaled_test_df <- scale_numeric_columns(test_df_reduced_2)
...
xgb_pred <- predict(xgb_best_fit, scaled_test_df)
lgbm_pred <- predict(lgbm_best_fit, scaled_test_df)
rf_pred <- predict(rf_best_fit, scaled_test_df)
lr_pred <- predict(lr_best_fit, scaled_test_df)
knn_pred <- predict(knn_best_fit, scaled_test_df)
```
...
pred_test <-  predict(meta_best_fit, new_data = prediction_stack_data)
```
