...
pacman::p_load(tidyverse,
               ...
               ggpmisc, # R squared value in plots
               plotly,
               vip,
               doParallel,
               tictoc)
...
```{r}
train_corr <- train_df_model %>% select(-id) %>% cor()
corrplot(train_corr)
```
...
```{r}
set.seed(123)
train_split <- initial_split(train_df_model %>% select(-id) , prop = 0.7, strata = score)
